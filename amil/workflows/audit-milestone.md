...
```bash
INIT=$(amil-utils orch init milestone-op)
...
```bash
integration_checker_model=$(amil-utils orch resolve-model amil-integration-checker --raw)
```
...
PHASE_INFO=$(amil-utils orch find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  amil-utils orch summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(amil-utils orch config get workflow.nyquist_validation --raw 2>/dev/null)
```
