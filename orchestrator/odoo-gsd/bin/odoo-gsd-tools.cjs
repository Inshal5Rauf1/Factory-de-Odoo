...
const moduleStatus = require('./lib/module-status.cjs');
const registryMod = require('./lib/registry.cjs');
const depGraph = require('./lib/dependency-graph.cjs');
const coherence = require('./lib/coherence.cjs');
...
async function main() {
  ...
  switch (command) {
    ...
    case 'dep-graph': {
      const subcommand = args[1];
      if (subcommand === 'build') {
        depGraph.cmdDepGraphBuild(cwd, raw);
      } else if (subcommand === 'order') {
        depGraph.cmdDepGraphOrder(cwd, raw);
      } else if (subcommand === 'tiers') {
        depGraph.cmdDepGraphTiers(cwd, raw);
      } else if (subcommand === 'can-generate') {
        depGraph.cmdDepGraphCanGenerate(cwd, args[2], raw);
      } else {
        error('Unknown dep-graph subcommand. Available: build, order, tiers, can-generate');
      }
      break;
    }
    ...
    case 'module-status': {
      const subcommand = args[1];
      if (subcommand === 'read') {
        moduleStatus.cmdModuleStatusRead(cwd, raw);
      } else if (subcommand === 'get') {
        moduleStatus.cmdModuleStatusGet(cwd, args[2], raw);
      } else if (subcommand === 'init') {
        moduleStatus.cmdModuleStatusInit(cwd, args[2], args[3], args[4], raw);
      } else if (subcommand === 'transition') {
        moduleStatus.cmdModuleStatusTransition(cwd, args[2], args[3], raw);
      } else if (subcommand === 'tiers') {
        moduleStatus.cmdTierStatus(cwd, raw);
      } else {
        error('Unknown module-status subcommand. Available: read, get, init, transition, tiers');
      }
      break;
    }
    ...
    case 'registry': {
      ...
      switch (registrySub) {
        case 'read':
          registryMod.cmdRegistryRead(cwd, raw);
          break;
        case 'read-model':
          registryMod.cmdRegistryReadModel(cwd, args[2], raw);
          break;
        case 'update':
          registryMod.cmdRegistryUpdate(cwd, args[2], raw);
          break;
        case 'rollback':
          registryMod.cmdRegistryRollback(cwd, raw);
          break;
        case 'validate':
          registryMod.cmdRegistryValidate(cwd, raw);
          break;
        case 'stats':
          registryMod.cmdRegistryStats(cwd, raw);
          break;
        default:
          error(`Unknown registry subcommand: ${registrySub}. Available: read, read-model, update, rollback, validate, stats`);
      }
      break;
    }
