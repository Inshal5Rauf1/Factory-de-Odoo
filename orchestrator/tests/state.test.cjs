...
describe('state-snapshot command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runAmilTools('state-snapshot', tmpDir);
...
`
    ...
    try {
      const result = runAmilTools(`state-snapshot --cwd "${tmpDir}"`, outsideDir);
      assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for invalid --cwd path', () => {
    ...
    const result = runAmilTools(`state-snapshot --cwd "${invalid}"`, tmpDir);
...
`
    ...
    const result = runAmilTools(['state', 'add-blocker', '--text', 'Waiting on vendor quote $1.00 before approval'], tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    const result = runAmilTools(`state add-blocker --text-file "${blockerPath}"`, tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('state json command', () => {
  ...
  test('missing STATE.md returns error', () => {
    const result = runAmilTools('state json', tmpDir);
    assert.ok(result.success, `Command should succeed: ${result.error}`);
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.amil_state_version, '1.0', 'should have version 1.0');
...
`
    ...
    const output = JSON.parse(result.output);
    assert.strictEqual(output.amil_state_version, '1.0', 'version from frontmatter');
...
`
    ...
    const result = runAmilTools('state update Status "Executing Plan 1"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const content = fs.readFileSync(path.join(tmpDir, '.planning', 'STATE.md'), 'utf-8');
    assert.ok(content.startsWith('---\n'), 'should start with frontmatter delimiter');
    assert.ok(content.includes('amil_state_version: 1.0'), 'should have version field');
...
`
    ...
    const result = runAmilTools('state patch --Status "In progress" --"Current Plan" 04-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`
    ...
    runAmilTools('state update Status "In progress"', tmpDir);
    runAmilTools('state update Status "Paused"', tmpDir);
...
`
    ...
    runAmilTools('state update Status "Executing Plan 5"', tmpDir);
...
describe('cmdStateLoad (state load)', () => {
  ...
  test('returns config and state when STATE.md exists', () => {
    ...
    const result = runAmilTools('state load', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns raw key=value format with --raw flag', () => {
    ...
    const result = runAmilTools('state load --raw', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    const result = runAmilTools('state get', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts bold field value', () => {
    ...
    const result = runAmilTools('state get Status', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('extracts markdown section content', () => {
    ...
    const result = runAmilTools('state get Blockers', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent field', () => {
    ...
    const result = runAmilTools('state get Missing', tmpDir);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch updates multiple fields at once', () => {
    ...
    const result = runAmilTools('state patch --Status Complete --"Current Phase" 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state patch reports failed fields that do not exist', () => {
    ...
    const result = runAmilTools('state patch --Status Done --Missing value', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update changes a single field', () => {
    ...
    const result = runAmilTools('state update Status "Phase complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('state update reports field not found', () => {
    ...
    const result = runAmilTools('state update Missing value', tmpDir);
  ...
  test('state update returns error when STATE.md missing', () => {
    const result = runAmilTools('state update Status value', tmpDir);
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    ...
    const result = runAmilTools('state advance-plan', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    ...
    const result = runAmilTools('state record-metric --phase 2 --plan 1 --duration 5min --tasks 3 --files 4', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    const result = runAmilTools('state record-metric --phase 1 --plan 1 --duration 2min --tasks 1 --files 2', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when required fields missing', () => {
    ...
    const result = runAmilTools('state record-metric --phase 1', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runAmilTools('state record-metric --phase 1 --plan 1 --duration 2min', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const result = runAmilTools('state update-progress', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    ...
    const result = runAmilTools('state resolve-blocker --text "api credentials"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    const result = runAmilTools('state resolve-blocker --text "single blocker"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error when text not provided', () => {
    ...
    const result = runAmilTools('state resolve-blocker', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns error when STATE.md missing', () => {
    const result = runAmilTools('state resolve-blocker --text "anything"', tmpDir);
    assert.ok(result.success, `Command should exit 0: ${result.error}`);
  ...
  test('returns resolved true even if no line matches', () => {
    ...
    const result = runAmilTools('state resolve-blocker --text "nonexistent blocker text"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates Last session timestamp even with no other options', () => {
    ...
    const result = runAmilTools('state record-session', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('sets Resume file to None when not specified', () => {
    ...
    const result = runAmilTools('state record-session --stopped-at "Phase 1 complete"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('milestone-scoped phase counting in frontmatter', () => {
  ...
  test('total_phases counts only current milestone phases', () => {
    ...
    const result = runAmilTools('state update Status "Executing"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    // Read the state json to check frontmatter
    const jsonResult = runAmilTools('state json', tmpDir);
  ...
  test('without ROADMAP counts all phases (pass-all filter)', () => {
    ...
    const result = runAmilTools('state update Status "In progress"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
