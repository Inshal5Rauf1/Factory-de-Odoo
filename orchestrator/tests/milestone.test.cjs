...
describe('milestone complete command', () => {
  ...
  test('archives roadmap, requirements, creates MILESTONES.md', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name MVP Foundation', tmpDir);
  ...
  test('prepends to existing MILESTONES.md (reverse chronological)', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name Beta', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('three sequential completions maintain reverse-chronological order', () => {
    ...
    let result = runAmilTools('milestone complete v1.1 --name Second', tmpDir);
    ...
    result = runAmilTools('milestone complete v1.2 --name Third', tmpDir);
  ...
  test('archives phase directories with --archive-phases flag', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name MVP --archive-phases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('archived REQUIREMENTS.md contains archive header', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name MVP', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('STATE.md gets updated during milestone complete', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name Test', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles missing ROADMAP.md gracefully', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name NoRoadmap', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('scopes stats to current milestone phases only', () => {
    ...
    const result = runAmilTools('milestone complete v1.1 --name "Second Release"', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('archive-phases only archives current milestone phases', () => {
    ...
    const result = runAmilTools('milestone complete v1.1 --name Test --archive-phases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('large phase numbers (456, 457) scope correctly', () => {
    ...
    const result = runAmilTools('milestone complete v1.49 --name DACP', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles empty phases directory', () => {
    ...
    const result = runAmilTools('milestone complete v1.0 --name EmptyPhases', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
`;
  ...
  test('marks single requirement complete (checkbox + table)', () => {
    ...
    const result = runAmilTools('requirements mark-complete TEST-01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('handles mixed prefixes in single call (TEST-XX, REG-XX, INFRA-XX)', () => {
    ...
    const result = runAmilTools('requirements mark-complete TEST-01,REG-01,INFRA-01', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('accepts space-separated IDs', () => {
    ...
    const result = runAmilTools('requirements mark-complete TEST-01 TEST-02', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns not_found for invalid IDs while updating valid ones', () => {
    ...
    const result = runAmilTools('requirements mark-complete TEST-01,FAKE-99', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('idempotent — re-marking already-complete requirement does not corrupt', () => {
    ...
    const result = runAmilTools('requirements mark-complete TEST-03', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
