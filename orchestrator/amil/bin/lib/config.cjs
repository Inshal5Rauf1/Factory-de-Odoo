...
function cmdConfigEnsureSection(cwd, raw) {
  ...
  // Load user-level defaults from ~/.amil/defaults.json if available
  const globalDefaultsPath = path.join(homedir, '.amil', 'defaults.json');
  let userDefaults = {};
  try {
    if (fs.existsSync(globalDefaultsPath)) {
      ...
      if ('depth' in userDefaults && !('granularity' in userDefaults)) {
        ...
        try { fs.writeFileSync(globalDefaultsPath, JSON.stringify(userDefaults, null, 2), 'utf-8'); } catch { /* best-effort migration write */ }
      }
    }
  } catch (err) {
