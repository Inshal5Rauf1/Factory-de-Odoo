...
```bash
TRANSITION=$(amil-utils orch phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(amil-utils orch progress bar --raw)
```
...
```bash
amil-utils orch config-set workflow._auto_chain_active false
```
