...
class TestRegistryIntegration:
    ...
    def test_registry_count_is_20(self):
        """After full import, exactly 20 preprocessors are registered."""
        entries = get_registered_preprocessors()
        assert len(entries) == 20, (
            f"Expected 20 registered preprocessors, got {len(entries)}: "
    ...
    def test_registry_order_matches_pipeline(self):
        ...
        entries = get_registered_preprocessors()
        orders = [e[0] for e in entries]
        expected = [5, 10, 12, 15, 22, 25, 27, 28, 30, 40, 50, 52, 60, 70, 80, 85, 90, 95, 100, 105]
    ...
    def test_auto_discovery_finds_all_modules(self):
        ...
        assert len(discovered) == 20, (
            f"Expected 20 discoverable modules, got {len(discovered)}: {discovered}"
        )
