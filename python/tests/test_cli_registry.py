...
class TestRegistryList:
    def test_registry_list_empty(self, runner, tmp_registry):
        with patch("amil_utils.commands.registry_cmds.find_registry_path", return_value=tmp_registry):
    ...
    def test_registry_list_with_modules(self, runner, populated_registry):
        with patch("amil_utils.commands.registry_cmds.find_registry_path", return_value=populated_registry):
            result = runner.invoke(main, ["registry", "list"])
        assert result.exit_code == 0
...
class TestPostRenderHook:
    def test_render_module_updates_registry(self, runner, tmp_path):
        ...
        with (
            patch("amil_utils.commands.render._find_registry_path", return_value=registry_path),
