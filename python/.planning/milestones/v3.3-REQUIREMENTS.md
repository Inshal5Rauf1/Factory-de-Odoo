...
**Defined:** 2026-03-07
**Core Value:** Compress months of repetitive Odoo module development into days by extending Amil's orchestration with Odoo-specialized agents, knowledge, and validation.
...
- [x] **ARCH-02**: Cross-module model registry with JSON persistence (`.amil-registry.json`), comodel validation, `depends` inference, cycle detection — called in CLI layer, not inside render pipeline
...
- [x] **ARCH-04**: Generation manifest with file paths, SHA256 checksums, template versions, preprocessor list — persisted as `.amil-manifest.json` sidecar
