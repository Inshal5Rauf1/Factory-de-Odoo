...
│       └── .env                 # Gitignored, local dev env vars (ODOO_DEV_PORT, etc.)
│
├── scripts/                     # Dev scripts
│   ├── odoo-dev.sh
│   └── verify-odoo-dev.py
...
| `pipeline/defaults.json` | `amil/defaults.json` | Referenced by 3 agents (scaffold, search, extend) — NOT used by Python code. Must be installed alongside agents at `~/.claude/amil/defaults.json`. |
...
Execute in this sequence to avoid mid-restructure broken state:

1. **Move Python package** (`pipeline/python/` → `python/`) and verify tests pass
2. **Move agents** (consolidate both directories into `agents/`)
3. **Move amil/ content** (workflows, references, templates, knowledge, defaults.json)
4. **Move commands and hooks**
5. **Move remaining files** (docker, scripts, assets, docs, .github, root-level docs)
6. **Update install.py** and verify global install works
7. **Merge CLAUDE.md** (three-way) and update README.md, CONTRIBUTING.md
8. **Merge .gitignore** (three-way) — key patterns to remap: `docker/dev/.env`, `.mcp.json`
9. **Delete stale files** and empty directories
10. **Create CONVENTIONS.md**
11. **Run full verification checklist**

## Files NOT Moved (remain in place)

These files already exist at root and are updated in place (not moved):
- `CLAUDE.md` — content updated (three-way merge), location unchanged
- `README.md` — content updated, location unchanged
- `.gitignore` — content updated (three-way merge), location unchanged
- `install.py` — content updated from `orchestrator/install.py`, location at root

## Notes for Implementers

- **Do NOT move `.venv/` directories** — they are untracked and will be recreated after `uv pip install -e python`
- **Clean build artifacts before committing:** remove `.coverage`, `.pytest_cache/`, `__pycache__/` from `python/` after move
- **Agent files verified:** agent `.md` files contain only `@~/.claude/` paths (not repo-relative paths). No content updates expected for agent files, but grep to confirm.
- **`.mcp.json`** should remain gitignored (contains localhost URLs). Ship as committed example or document in README.
- **`.planning/` directories** will be removed along with `orchestrator/` and `pipeline/`. Root `.planning/` remains as canonical location.
...
8. Grep for stale references — zero remaining:
   ```bash
   grep -r "pipeline/" --include="*.md" --include="*.py" --include="*.yml" --include="*.json" . | grep -v ".git/" | grep -v ".venv/"
   grep -r "orchestrator/" --include="*.md" --include="*.py" --include="*.yml" --include="*.json" . | grep -v ".git/" | grep -v ".venv/"
   ```
9. `CONVENTIONS.md` exists at root
10. `uv run pytest tests/ --collect-only -q | tail -1` — confirm exact test count post-move
